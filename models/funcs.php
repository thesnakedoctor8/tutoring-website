<?php
	...
	function deleteUsers($users)
	{
		...
		$stmt2 = $mysqli->prepare("DELETE FROM ".$db_table_prefix."user_permission_matches 
			WHERE user_id = ?");
		$stmt3 = $mysqli->prepare("DELETE FROM ".$db_table_prefix."subscriptions 
			WHERE user_id = ?");
	...
	//Functions that interact mainly with .subjects table
	//------------------------------------------------------------------------------

	//Create a subject in DB
	function createSubject($name, $price, $description)
	{
		global $mysqli, $db_table_prefix; 
		$stmt = $mysqli->prepare("INSERT INTO ".$db_table_prefix."subjects (
			name,
			price,
			description
			)
	...
	//Retrieve names of all the subjects
	function fetchAllSubjectNames()
	{
		global $mysqli, $db_table_prefix; 
		$stmt = $mysqli->prepare("SELECT 
			name
			FROM ".$db_table_prefix."subjects");
		$stmt->execute();
	...
	//Retrieve information for all subjects
	...
	//Check if a subject name exists in the DB
	function subjectNameExists($subject)
	{
		global $mysqli, $db_table_prefix;
		$stmt = $mysqli->prepare("SELECT
			id
	...
	//Functions that interact mainly with .subscriptions table
	//------------------------------------------------------------------------------

	//Add a Subscription in DB
	function addSubscription($user_id, $subject_id)
	{
		global $mysqli, $db_table_prefix; 
		$subscriptions = fetchAllSubscriptions($user_id);
		...
		$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."subscriptions
			...
			user_id = ?
			LIMIT 1");
		$stmt->bind_param("si", $subscriptions, $user_id);
		$result = $stmt->execute();
		$stmt->close();
		return $result;
	}
	...
	//Delete a subject from the DB
	function deleteSubscription($user_id, $subject_id)
	{
		global $mysqli, $db_table_prefix; 
		$subscriptions = fetchAllSubscriptions($user_id);
		if($subscriptions == null)
		...
		$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."subscriptions
			...
			user_id = ?
			LIMIT 1");
		$stmt->bind_param("si", $edit, $user_id);
		$result = $stmt->execute();
		$stmt->close();
		return $result;
	}
	...
	function fetchAllSubscriptions($user_id)
	{
		global $mysqli, $db_table_prefix; 
		$stmt = $mysqli->prepare("SELECT 
			subscriptions
	...
	//Retrieve all Subscriptions
	function alreadySubscribed($user_id, $subject_id)
	{
		global $mysqli, $db_table_prefix; 
	...
	{	
		...
		else
		{
			...
			else if ($loggedInUser->user_id == $master_account)
			{
				return true;
			}
			else {
